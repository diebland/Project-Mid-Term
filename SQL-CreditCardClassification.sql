...
COUNT(*) OVER (PARTITION BY bank_accounts_open) 
FROM credit_card_data
GROUP BY credit_cards_held
ORDER BY credit_cards_held ;

SELECT bank_accounts_open, 
COUNT(*) OVER (PARTITION BY credit_cards_held) 
FROM credit_card_data
GROUP BY bank_accounts_open
ORDER BY bank_accounts_open ;
SELECT bank_accounts_open, credit_cards_held
FROM credit_card_data;
...
SELECT ROUND(AVG(average_balance),2), credit_rating FROM credit_card_data
GROUP BY credit_rating;

create or replace view Customers__Balance_high as
SELECT ROUND(AVG(average_balance),2) avg_balance_high
FROM credit_card_data
WHERE credit_rating = "High";

create or replace view Customers__Balance_medium as
SELECT ROUND(AVG(average_balance),2) avg_balance_medium
FROM credit_card_data
WHERE credit_rating = "Medium";


SELECT Customers__Balance_high.customer, avg_balance_high, avg_balance_medium, Customers__Balance_high.avg_balance,  Customers__Balance_medium.avg_balance
FROM Customers__Balance_high
LEFT JOIN Customers__Balance_medium
ON Customers__Balance_high.avg_balance= Customers__Balance_medium.avg_balance;
